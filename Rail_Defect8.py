    ...
    if dark_spots:
        while True:
            ...
            layout_lst = [[sg.Text('Rail',size=(20, 1), font='Lucida',justification='left')],
                            ...
                            [sg.Button('Remove', font=('Times New Roman',12)),sg.Button('Cancel', font=('Times New Roman',12)),sg.Button('Next', font=('Times New Roman',12))]]
            ...
            #=-=-=-=-=-=-=-=-=-=-=-=
            #(x, y, w, h, dimensions) = dark_spots_dict[int(v['list1'][0])]
            #temp_image = image_mini.copy()
            #cv2.rectangle(temp_image, (x + frame_start[0], y + frame_start[1]), (x + frame_start[0] + w, y + frame_start[1] + h), (0, 0, 255), 2)
            #=-=-=-=-=-=-=-=-=-=-=-=

            if e == 'Remove':
                names.remove(v['list1'][0])
                ##val = window_list('list1').get()[0]
                ##names.remove(val)
                window_list['list1'].update(names)
                #msg = "A new item removed : {}".format(val)
                #window['-MSG-'].update(msg)
                #window_list.close()
                #window_list.FindElement('list1').Update(names)
            elif e == 'Cancel':
                window_list.close()
            elif e == 'Next':
                pass
